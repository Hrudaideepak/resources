...
import { getSubjectBySlug, getSubjectResources } from "@/lib/repo";
...
export default async function SubjectPage({
  ...
  const all = await getSubjectResources(subject.id);
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        {unitsPresent.length > 0 && (
          <>
            ...
            {[1, 2, 3, 4, 5].map((u) => (
              <Link key={u} href={q({ unit: u })} className={`rounded-full px-3 py-1 ${unitFilter === u ? "bg-stone-900 text-white" : "bg-stone-100 hover:bg-stone-200"}`}>
                {u}
              </Link>
            ))}
          </>
